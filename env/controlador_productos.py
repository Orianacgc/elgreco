...
def obtener_producto_categoria():
    ...
    elgreco = []
    with conexion.cursor() as cursor:
        cursor.execute("SELECT idproducto,codigo,nombre,stock,p.descripcion,color,medida,imagen,costo_venta,estado,c.descripcion FROM producto p inner join categoria c on p.idcategoria=c.idcategoria order by  c.descripcion, nombre")
